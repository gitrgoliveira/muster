...
- Request body unchanged, plus **optional** additive field `chain` (ordered step list) to override the default chain, e.g. `"chain": [{"name":"plan","permissionMode":"plan"}, {"name":"build","permissionMode":"acceptEdits"}]` (`name` required; `permissionMode` required per step — never silently defaulted, FR-012a; `promptRef` optional).
