...
- **A `skill:<id>` label is malformed** (empty id after the prefix, or extra colons like `skill:a:b`): it MUST be ignored with a warning, never crash assembly or resolve to a bogus skill.
...
- **FR-004**: Assembly MUST include a summary of each prior `done` or `failed` step in the current run's chain, sourced from that step's runlog, before the current step's own prompt content.
...
- **FR-016**: Imported skills MUST be stored as local file(s), not a new Dolt table and not through `bd` — same storage-class rationale as FR-008.
...
- **FR-024**: Muster MUST expose `POST /api/v1/memories/prime { beadID }`, which associates a snapshot of current memories with the named bead such that its *next* dispatch's assembled prompt includes a "Primed memories" section.
