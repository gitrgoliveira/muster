...
3. **Given** the bead's prefix has no configured repo mapping, **When** a user dispatches it, **Then** muster returns `422 UNPROCESSABLE_ENTITY` (`UNMAPPED_PREFIX`) with `no source repo configured for prefix "<prefix>"` and launches nothing.
