...
- Fallback (tmux absent): `runlog.line` still flows from the child's stdout/stderr; `tmux.session.*` still bracket the run using the **same canonical `muster/<bead>/<step>/<loop>` `session` name** as the tmux path (so `session` is a stable correlation key across both transports); no catch-up.
